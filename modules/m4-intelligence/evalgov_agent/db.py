...
class AgentDB:
    ...
    def get_all_active_findings(self, hours: int = 24) -> list[dict]:
        return self._run(
            f"SELECT finding_id, finding_type, severity, title, summary, rca, "
            f"recommendation, signal_data, affected_agent, status, created_at "
            f"FROM otel.gov_agent_findings FINAL "
            f"WHERE status IN ('active', 'acknowledged') "
            f"AND created_at >= now() - INTERVAL {int(hours)} HOUR "
            f"ORDER BY status ASC, created_at DESC LIMIT 100"
        )
    ...
    def get_key_rejection_events(
        self, key_id: str = "", hours: int = 24, limit: int = 100
    ) -> list[dict]:
        h = int(hours)
        conds = [f"ts >= now() - INTERVAL {h} HOUR"]
    ...
    def get_gateway_key_events_live(self, hours: int = 1, limit: int = 25) -> list[dict]:
        """Return recent key rejection events (rate limit, budget alert, model not allowed)."""
        try:
            return self._run(
                f"SELECT ts, key_prefix, agent_role, event_type, model_requested, detail "
                f"FROM otel.gateway_key_events "
                f"WHERE event_type IN ('rate_limit', 'budget_alert', 'model_not_allowed') "
    ...
    def get_pending_change_proposals(self, limit: int = 25) -> list[dict]:
        """Return pending gateway change proposals awaiting operator review."""
        try:
            return self._run(
                f"SELECT change_id, change_type, agent_role, system_id, "
                f"description, proposed_by, proposed_at "
                f"FROM otel.gateway_proposed_changes FINAL "
                f"WHERE status = 'pending' "
                f"ORDER BY proposed_at DESC LIMIT {int(limit)}"
            )
        except Exception:
            return []
    ...
    def get_gateway_call_stats(
        self, hours: int = 168, agent_role: str = ""
    ) -> dict:
        """Aggregate gateway call stats per (agent_role, model_used) joined with eval scores."""
        h = int(hours)
        role_cond = f"AND agent_role = '{agent_role.replace(chr(39), '')}'" if agent_role else ""

        # Per-model call volume, latency, tokens
        ...
        # Per-model eval scores — LEFT JOIN on trace_id (exact match, no stale-gateway risk)
        score_rows = []
        try:
    ...
    def compare_shadow_vs_primary(self, agent_role: str = "", hours: int = 168) -> dict:
        """Compare shadow eval scores against primary eval scores for the same agent role."""
        h = int(hours)
        ...
        # Shadow call volume + latency per model
        shadow_calls = []
        try:
        ...
        # Shadow eval scores from gateway_shadow_evals
        shadow_scores = []
        try:
        ...
        # Primary eval scores (trace_id join — only evals that went through the gateway)
        primary_scores = []
        try:
