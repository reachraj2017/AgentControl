...
class Repository:
    ...
    def get_prompt_evals(
        ...
        """Return recent prompt eval rows, enriched with gateway model_used when available."""
        params: dict = {"limit": limit, "offset": offset}
        where_inner = "WHERE pe.agent_name = %(agent_name)s" if agent_name else ""
        where_base  = "WHERE agent_name = %(agent_name)s"    if agent_name else ""
        if agent_name:
            params["agent_name"] = agent_name

        gw_query = f"""
            WITH enriched AS (
                SELECT pe.prompt_eval_id, pe.prompt_hash, pe.trace_id, pe.run_id,
                       pe.agent_name, pe.model,
                       if(gw.model_used != '' AND gw.model_used IS NOT NULL,
                          gw.model_used, pe.model) AS model_used,
                       pe.prompt_text, pe.response_text,
                       pe.latency_ms, pe.prompt_tokens, pe.completion_tokens,
                       pe.scores, pe.created_at
        ...
        """
        base_query = f"""
            SELECT prompt_eval_id, prompt_hash, trace_id, run_id,
                   agent_name, model, model AS model_used,
        ...
        """
        try:
            return self._ch.fetch_all(gw_query, params)
        except Exception as exc:
            log.warning("get_prompt_evals_gw_join_failed, falling back: %s", exc)
            try:
                return self._ch.fetch_all(base_query, params)
            except Exception as exc2:
                log.error("get_prompt_evals_failed", error=str(exc2))
                return []

    # ------------------------------------------------------------------
    # Shadow eval (scores for gateway shadow calls)
    # ------------------------------------------------------------------

    def get_unscored_shadow_rows(self, batch_size: int = 20) -> list[dict]:
        """Return shadow call rows that have not yet been scored."""
        query = """
            SELECT call_id, agent_role, model_used, prompt_text, response_text
            FROM otel.gateway_call_log
            WHERE is_shadow = 1
              AND prompt_text  != ''
              AND response_text != ''
              AND created_at >= now() - INTERVAL 7 DAY
              AND call_id NOT IN (SELECT call_id FROM otel.gateway_shadow_evals)
            ORDER BY created_at ASC
            LIMIT %(limit)s
        """
        try:
            return self._ch.fetch_all(query, {"limit": batch_size})
        except Exception as exc:
            log.warning("get_unscored_shadow_rows_failed", error=str(exc))
            return []

    def save_shadow_eval_scores(
        self,
        call_id: str,
        agent_role: str,
        model_used: str,
        ...
        query = """
            INSERT INTO otel.gateway_shadow_evals
                (call_id, agent_role, model_used, prompt_text, response_text, scores)
            VALUES
        """
        data = [(
            call_id, agent_role, model_used,
            prompt_text[:4000], response_text[:4000],
            {k: float(v) for k, v in scores.items()},
        )]
        try:
            self._ch.execute_many(query, data)
            log.info("shadow_eval_saved", call_id=call_id, scores=list(scores.keys()))
        except Exception as exc:
            log.error("save_shadow_eval_scores_failed", call_id=call_id, error=str(exc))
    ...
    # ------------------------------------------------------------------
    # Gateway ingest (GatewayIngestPipeline)
    # ------------------------------------------------------------------

    def get_pending_gateway_calls(self, batch_size: int = 50) -> list[dict]:
        """
        Return gateway_call_log rows not yet processed by GatewayIngestPipeline.

        Joined against the gateway_call_eval_state sidecar table (state IS
        NULL or 'pending'). gateway_call_log itself is never mutated — it's
        the immutable durable log; eval_state is the ingestion checkpoint.
        """
        query = """
            SELECT
                gcl.call_id            AS call_id,
                gcl.trace_id            AS trace_id,
                gcl.run_id              AS run_id,
                gcl.conversation_id     AS conversation_id,
                gcl.system_id           AS system_id,
                gcl.agent_role          AS agent_role,
                gcl.model_used          AS model_used,
                gcl.protocol            AS protocol,
                gcl.prompt_text         AS prompt_text,
                gcl.response_text       AS response_text,
                gcl.messages_json       AS messages_json,
                gcl.response_tool_calls_json AS response_tool_calls_json,
                gcl.tokens_in           AS tokens_in,
                gcl.tokens_out          AS tokens_out,
                gcl.latency_ms          AS latency_ms,
                gcl.started_at          AS started_at,
                gcl.ended_at            AS ended_at,
                gcl.status              AS status,
                gcl.created_at          AS created_at
            FROM otel.gateway_call_log AS gcl
            LEFT JOIN (
                SELECT call_id, argMax(state, updated_at) AS state
                FROM otel.gateway_call_eval_state
                GROUP BY call_id
            ) AS st ON gcl.call_id = st.call_id
            WHERE gcl.is_shadow = 0
              AND (st.state = '' OR st.state = 'pending')
        ...
        try:
            ...
            log.warning("get_pending_gateway_calls_failed", error=str(exc))
            return []

    def get_pending_structural_events(self, conversation_ids: list[str], run_ids: list[str]) -> list[dict]:
        """
        Return gateway_structural_events rows (handoff / tool_span / checkpoint)
        for the given conversation_id / run_id keys, for folding into target
        synthesis alongside the same batch's gateway_call_log rows.
        """
        if not conversation_ids and not run_ids:
            return []
        conditions = []
        params: dict = {}
        if conversation_ids:
            conditions.append("conversation_id IN %(conv_ids)s")
            params["conv_ids"] = tuple(conversation_ids) or ("",)
        if run_ids:
            conditions.append("run_id IN %(run_ids)s")
            params["run_ids"] = tuple(run_ids) or ("",)
        where = " OR ".join(conditions)
        query = f"""
            SELECT event_id, call_type, conversation_id, run_id, system_id,
                   agent_role, payload_json, created_at
            FROM otel.gateway_structural_events
            WHERE {where}
            ORDER BY created_at ASC
        """
        try:
            ...
            log.warning("get_pending_structural_events_failed", error=str(exc))
            return []

    def mark_gateway_calls(self, call_ids: list[str], state: str, trace_id: str = "") -> None:
        """Write ingestion-state rows for a batch of gateway_call_log call_ids.

        Never mutates gateway_call_log (immutable log) — writes to the
        ReplacingMergeTree sidecar table instead, one row per call_id per
        state transition.
        """
        if not call_ids:
            return
        now = datetime.now(timezone.utc)
        query = """
            INSERT INTO otel.gateway_call_eval_state
                (call_id, state, trace_id, updated_at)
            VALUES
        """
        data = [(cid, state, trace_id, now) for cid in call_ids]
        try:
            self._ch.execute_many(query, data)
        except Exception as exc:
            log.error("mark_gateway_calls_failed", state=state, count=len(call_ids), error=str(exc))

    def trace_has_inprocess_spans(self, trace_id: str) -> bool:
        """
        True if a non-gateway-synthesized agent-task span already exists for
        this trace_id in otel_traces — i.e. an in-process tracer (ACP SDK,
        OpenInference/OpenLLMetry auto-instrumentor, or native OTel GenAI
        export) already produced this trace. GatewayIngestPipeline uses this
        as the double-eval guard: MERGE mode (attach metadata only) instead
        of SYNTHESISE + re-evaluate.
        """
        if not trace_id:
            return False
        query = """
        ...
        """
        try:
            spans = self._ch.fetch_all(query, {"trace_id": trace_id})
        except Exception as exc:
            log.warning("trace_has_inprocess_spans_failed", trace_id=trace_id, error=str(exc))
            return False
        if not spans:
            return False
        # Local import avoids a module-load-order cycle (ingestion imports db.repository).
        from ingestion.semconv_mapping import is_agent_task_span
        return any(is_agent_task_span(s) for s in spans)
