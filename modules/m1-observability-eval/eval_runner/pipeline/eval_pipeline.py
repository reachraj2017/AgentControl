...
class EvalPipeline:
    ...
    def __init__(
        ...
        # Load sampling rate from gov_threshold_config on startup (overrides YAML)
        self._load_sampling_rate_from_db()

    def _load_sampling_rate_from_db(self) -> None:
        """Override YAML sampling rate with value from gov_threshold_config if set."""
        try:
            ...
            if rows:
                db_rate = float(rows[0]["value"])
                log.info("sampling_rate_from_db", rate=db_rate,
                         yaml_rate=self._sampling_rate)
                self._sampling_rate = db_rate
        except Exception as exc:
            log.debug("sampling_rate_db_read_skipped", error=str(exc))
    ...
    def run_from_targets(
        ...
        """
        Evaluate a pre-assembled target set (task_spans/tool_spans/handoff_spans/
        llm_spans/all_spans) without re-querying/re-assembling spans.

        Used by:
          - _run_inner() for the in-process (OTLP span) path
          - GatewayIngestPipeline for gateway-only / synthesised traces
        Identical evaluator cascade + writes to eval_scores / prompt_evals
        regardless of which caller produced the target set.
        """
