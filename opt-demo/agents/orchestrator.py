...
Phase 1: pure ADK, no ACP imports.
ACP governance is injected via set_governance_client() from acp_setup.py.
"""
...
# ── Module-level governance client — injected by acp_setup.py ────────────────
_gov_client = None


def set_governance_client(client) -> None:
    """Inject the ACP GovernanceClient so _before_tool_cb can use it."""
    global _gov_client
    _gov_client = client
...
def _make_model(agent_role: str = "unknown") -> LiteLlm:
    if _OLLAMA_ENABLED:
    ...
    return LiteLlm(
        ...
        extra_headers={
            ...
            "X-Gateway-System-Id":  "opt-demo",
        },
    )


def _no_think(instruction: str) -> str:
    """Append /no_think directive when running on Ollama to suppress thinking output."""
    if _OLLAMA_ENABLED:
        return instruction + "\n\n/no_think"
    return instruction
...
def _before_tool_cb(tool, args, tool_context):
    """Block transfer_to_agent; governance gate-check all real tool calls."""
    ...
    # ACP M2: governance gate check before tool execution
    if _gov_client is not None:
        try:
            result = _gov_client.check_policy(
                "tool_call",
                {"tool_name": tool_name, "agent_role": "orchestrator"},
            )
            if result.get("decision") == "block":
                message = result.get("reason", "Policy blocked tool execution.")
                return {"result": f"[GOVERNANCE BLOCK] {message}"}
        except Exception:
...
def create_searcher_agent() -> LlmAgent:
    return LlmAgent(
        ...
        model=_make_model("searcher"),
        instruction=_no_think(
            f"IMPORTANT: Today's date is {_today()}. "
            "You are a web search specialist. Use the web_search tool to find accurate, up-to-date information.\n\n"
            ...
            "CRITICAL: If web_search returns '[GOVERNANCE BLOCK]', relay that message exactly and stop."
        ),
        tools=[web_search],
