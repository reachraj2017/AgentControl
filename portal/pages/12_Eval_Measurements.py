...
with tab_pa:
    ...
    with st.expander("Filters", expanded=True):
        ...
        with pf2:
            try:
                pa_models_raw = _pa_q(
                    "SELECT DISTINCT model_used AS m FROM otel.gateway_call_log "
                    "WHERE is_shadow = 0 AND model_used != '' "
                    "UNION DISTINCT "
                    "SELECT DISTINCT model AS m FROM otel.prompt_evals WHERE model != '' "
                    "ORDER BY m"
                )
            except Exception:
                pa_models_raw = _pa_q("SELECT DISTINCT model AS m FROM otel.prompt_evals WHERE model != '' ORDER BY m")
            pa_model_opts = ["(all)"] + [r["m"] for r in pa_models_raw]
    ...
    _time_cond = f"created_at >= '{_pa_ts}' AND created_at <= '{_pa_te}'"
    _pa_where      = [_time_cond]   # for gateway CTE query (has model_used column)
    _pa_where_base = [_time_cond]   # for fallback base query (only model column)
    ...
    if pa_sel_source != "(all)":
        ...
        _pa_where.append(_src_cond)
        _pa_where_base.append(_src_cond)
    if pa_sel_agent != "(all)":
        _pa_where.append(f"agent_name = '{pa_sel_agent}'")
        _pa_where_base.append(f"agent_name = '{pa_sel_agent}'")
    if pa_sel_model != "(all)":
        _safe_model = pa_sel_model.replace("'", "\\'")
        _pa_where.append(f"(model = '{_safe_model}' OR model_used = '{_safe_model}')")
        _pa_where_base.append(f"model = '{_safe_model}'")
    if pa_sel_conv.strip():
        ...
        _pa_where.append(_conv_cond)
        _pa_where_base.append(_conv_cond)
    if pa_score_metric != "(none)":
        _score_cond = f"scores['{pa_score_metric}'] <= {pa_max_score}"
        _pa_where.append(_score_cond)
        _pa_where_base.append(_score_cond)

    _pa_where_clause      = "WHERE " + " AND ".join(_pa_where)
    _pa_base_where_clause = "WHERE " + " AND ".join(_pa_where_base)
    _pa_order_clause  = (
    ...
    _pa_gw_query = f"""
        WITH enriched AS (
            SELECT pe.prompt_eval_id, pe.prompt_hash, pe.trace_id, pe.span_id, pe.run_id,
            ...
            LEFT JOIN (
                SELECT trace_id, agent_role,
                       argMax(model_used, created_at) AS model_used
                FROM otel.gateway_call_log
                WHERE is_shadow = 0 AND trace_id != ''
                GROUP BY trace_id, agent_role
            ) gw ON pe.trace_id = gw.trace_id AND gw.agent_role = pe.agent_name
        )
    ...
    """
    try:
        pa_rows = _pa_q(_pa_gw_query)
    except Exception:
        pa_rows = _pa_q(_pa_base_query)
    ...
    if not pa_rows:
        ...
        _pa_total_cost = sum(
            (r.get("prompt_tokens", 0) or 0) * _input_rate +
            (r.get("completion_tokens", 0) or 0) * _output_rate
            for r in pa_rows
        )
        ...
        for r in pa_rows:
            ...
            _model_req  = r.get("model", "")
            _model_used = r.get("model_used") or _model_req
            _model_disp = (
                f"{_model_used} (req: {_model_req})"
                if _model_used and _model_used != _model_req
                else _model_used or _model_req
            )
            rec    = {
                ...
                "est_cost_usd":    round(p_tok * _input_rate + c_tok * _output_rate, 6),
...
with tab_thresh:
    ...
    # ── LLM judge sampling rate ───────────────────────────────────────────────
    with st.expander("⚙️ **Eval Sampling Settings**", expanded=False):
        try:
            _sr_rows = db._execute(
                "SELECT value FROM otel.gov_threshold_config FINAL "
                "WHERE config_key = 'sampling.online_llm_judge_rate'"
            )
            _current_rate = float(_sr_rows[0]["value"]) if _sr_rows else None
        except Exception:
            _current_rate = None

        _yaml_rate = 1.0  # what the YAML currently says
        _display_rate = _current_rate if _current_rate is not None else _yaml_rate

        st.markdown("**LLM judge sampling rate** — fraction of traces that run expensive LLM judges (faithfulness, hallucination, toxicity, etc.). Tier-1 metrics always run on every trace.")
        st.caption(f"Current: **{_display_rate:.0%}** {'(from database — overrides YAML)' if _current_rate is not None else '(from YAML default)'}")

        _col_slider, _col_btn = st.columns([4, 1])
        _new_rate = _col_slider.slider(
            "Sampling rate",
            min_value=0.05, max_value=1.0, step=0.05,
            value=_display_rate,
            format="%.0f%%",
            key="sampling_rate_slider",
            label_visibility="collapsed",
        )
        if _col_btn.button("Save", key="save_sampling_rate", type="primary", use_container_width=True):
            try:
                db._execute_no_result(
                    "INSERT INTO otel.gov_threshold_config "
                    "(config_key, display_name, category, value, unit, description, updated_by) VALUES",
                    [(
                        "sampling.online_llm_judge_rate",
                        "LLM Judge Sampling Rate",
                        "sampling",
                        float(_new_rate),
                        "ratio",
                        "Fraction of online traces that run LLM judge evaluators. "
                        "Tier-1 metrics are unaffected. Restart eval-runner to apply.",
                        "portal",
                    )],
                )
                st.success(f"Saved {_new_rate:.0%}. Restart eval-runner to apply: `docker compose restart eval-runner`")
                st.rerun()
            except Exception as _se:
                st.error(f"Save failed: {_se}")

        st.caption("⚠️ Change takes effect after eval-runner restarts (it reads this value at startup).")
