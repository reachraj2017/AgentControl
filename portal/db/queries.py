...
class ClickHouseQueries:
    ...
    def get_prompts_for_run(self, run_id: str, limit: int = 200) -> list[dict]:
        """Return prompt_evals rows for a specific run_id, enriched with gateway model if available."""
        params = {"run_id": run_id, "limit": limit}
        gw_query = (
            "WITH enriched AS ("
            "  SELECT pe.prompt_eval_id, pe.prompt_hash, pe.trace_id, pe.run_id, pe.agent_name,"
            "         pe.model,"
            "         if(gw.model_used != '' AND gw.model_used IS NOT NULL,"
            "            gw.model_used, pe.model) AS model_used,"
            "         pe.prompt_text, pe.response_text, pe.latency_ms,"
            "         pe.prompt_tokens, pe.completion_tokens, pe.scores, pe.created_at"
            "  FROM otel.prompt_evals pe"
            "  LEFT JOIN ("
            "    SELECT trace_id, model_used"
            "    FROM otel.gateway_call_log WHERE is_shadow = 0 AND trace_id != ''"
        ...
        try:
            return self._execute(gw_query, params)
        except Exception:
            return self._execute(base_query, params)
    ...
    def get_key_events(self, key_id: str = "", limit: int = 200, hours: int = 24) -> list[dict]:
        conditions = [f"ts >= now() - INTERVAL {int(hours)} HOUR"]
        if key_id:
            conditions.append("key_id = %(kid)s")
        where = "WHERE " + " AND ".join(conditions)
        try:
            return self._execute(
                ...
                f"ORDER BY ts DESC LIMIT %(limit)s",
                {"kid": key_id, "limit": limit},
            )
        except Exception:
            return []

    def get_known_agent_roles(self) -> list[str]:
        """Return distinct agent roles seen in gateway traffic, sorted, with '*' prepended."""
        _fallback = ["*", "orchestrator", "searcher", "summarizer", "translator", "unknown"]
        try:
            rows = self._execute(
                "SELECT DISTINCT agent_role FROM otel.gateway_call_log "
                "WHERE agent_role != '' ORDER BY agent_role"
            )
            roles = [r["agent_role"] for r in rows if r.get("agent_role")]
            if not roles:
                return _fallback
            # always include '*' first and 'unknown' if not already present
