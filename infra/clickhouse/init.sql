...
-- Gateway key rejection / block events (auth failures, rate limits, budget blocks)
...
-- See design/v2-gateway-capture-m1-ingest.md and design/checkpoint-handoff-ingest.md.
-- gateway_call_log itself gains new columns (conversation_id, protocol,
-- started_at, ended_at, messages_json, response_tool_calls_json) via
-- modules/m3-agent-gateway/agent_gateway/db.py — that module owns the
-- gateway_* table family's DDL/ALTER lifecycle. The two tables below are
-- declared here too (IF NOT EXISTS) since M1's eval-runner (a separate
-- service/container) also needs them present at its own startup.

-- Ingest-state cursor for M1's GatewayIngestPipeline — which gateway_call_log
-- rows have already been evaluated. Written by M1, never by the gateway.
...
-- Checkpoint / handoff / tool-span records — the non-LLM-call signals from
-- design/checkpoint-handoff-ingest.md. Written by the gateway's
-- /v1/checkpoint, /v1/handoff, /v1/tool-span endpoints; read by M1's
-- GatewayIngestPipeline to synthesise handoff_spans / tool_spans.
